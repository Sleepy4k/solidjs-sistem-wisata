...
const cardMetaData = (cardName: string) => {
  switch (cardName.toLowerCase()) {
    ...
    default:
      return { color: "gray", icon: faMoneyBill, isCurrency: false };
  }
};
...
const Business: Component = () => {
  ...
  let modalForm: HTMLFormElement, modalFieldContainer: HTMLDivElement;
  ...
  const unformatNumber = (val: string): string => val.replace(/\./g, "");

  const fullUnformatNumber = (val: string): string => {
  ...
  const inputCls =
    "w-full px-2.5 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white";

  const populateFormFields = (fields: IField[], values?: Record<string, any>) => {
        ...
        if (el) {
          if ((el as HTMLInputElement).type === "checkbox") {
            ...
            if (field.type === "number" || field.type === "currency") {
              const unformatted = fullUnformatNumber(
                String(values[field.name] ?? "")
              );
              (el as HTMLInputElement).value = unformatted;
            } else {
  ...
  const handleFormSubmit = async (e: Event) => {
    ...
    try {
      ...
      formData.forEach((value, key) => {
        const el = modalForm.querySelector(`[name="${key}"]`) as
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const id = selectedRow()?.id;
      const endpoint = `/dashboard/${params.role}/${params.slug}/${id}`;
    ...
    <>
                      ...
                      <Match when={field.type === "select"}>
                        ...
                        >
                          ...
                          <For each={field.options}>
                            {(option) => <option value={option.value}>{ucWords(option.label)}</option>}
                          </For>
                        </select>
                      </Match>
      ...
      <Show when={showAdd() || showEdit()}>
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) { setShowAdd(false); setShowEdit(false); }
          }}
        >
